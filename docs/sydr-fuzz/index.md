...
Доступные опции:
    ...
    Commands:
      run         Run hybrid fuzzing with Sydr and libFuzzer/AFL++/Honggfuzz/LibAFL-DiFuzz, Python
...
### Опции для создания шаблона конфигурационного TOML файла
    ...
    Options:
      ...
      -c, --corpus <corpus>                   Path to corpus directory
          ...
          --python-afl [<RUN_TARGET_LINE>...] Add python-afl table to TOML config template
...
Опция **--python-afl** добавляет таблицу pyafl (несовместима с другими таблицами).
...
### Опции запуска фаззинга
    ...
    $ sydr-fuzz run -h
    Run hybrid fuzzing with Sydr and libFuzzer/AFL++/Honggfuzz/LibAFL-DiFuzz, Python fuzzing with Atheris/python-afl,
...
`honggfuzz`, `atheris`, `jazzer`, `jazzer.js`, `luzer` или `difuzz`.
