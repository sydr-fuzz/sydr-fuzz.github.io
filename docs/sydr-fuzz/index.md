...
[AFL++](https://aflplus.plus/), [Honggfuzz](https://honggfuzz.dev/), LibAFL-DiFuzz и Crusher,
...
- Гибридный фаззинг с помощью Sydr и AFL++/libFuzzer/Honggfuzz/LibAFL-DiFuzz/Crusher,
...
Доступные опции:
    ...
    Commands:
      run         Run hybrid fuzzing with Sydr and
                      libFuzzer/AFL++/Honggfuzz/LibAFL-DiFuzz/Crusher, Python fuzzing with
                      Atheris/python-afl, Java fuzzing with Jazzer, JavaScript fuzzing with
                      Jazzer.js, or C# fuzzing with Sharpfuzz
...
### Опции для создания шаблона конфигурационного TOML файла
    ...
    Options:
          ...
          --difuzz [<RUN_TARGET_LINE>...]      Add LibAFL-DiFuzz table to TOML config template
          --crusher [<RUN_TARGET_LINE>...]     Add Crusher table to TOML config template
      ...
      -c, --corpus <corpus>                    Path to corpus directory
          ...
          --python-afl [<RUN_TARGET_LINE>...]  Add pyafl table to TOML config template
...
Опция **--difuzz** добавляет таблицу LibAFL-DiFuzz.

Опция **--crusher** добавляет таблицу Crusher.

Опция **--sydr** добавляет таблицу Sydr.

Опция **--cov**  добавляет таблицу для сбора покрытия (для компилируемых языков или C#)
Может быть указана только вместе с одной из таблиц AFL++, libFuzzer, LibAFL-DiFuzz, Sydr, Sharpfuzz.
...
Опция **--python-afl** добавляет таблицу pyafl (несовместима с другими таблицами, кроме [cov]).
...
### Опции запуска фаззинга
    ...
    $ sydr-fuzz run -h
    Run hybrid fuzzing with Sydr and libFuzzer/AFL++/Honggfuzz/LibAFL-DiFuzz/Crusher, Python
    fuzzing with Atheris/python-afl, Java fuzzing with Jazzer, JavaScript fuzzing with
    Jazzer.js, or C# fuzzing with Sharpfuzz
...
Нижеперечисленные опции поддерживаются только для гибридного фаззинга с Sydr.
...
### Опции минимизации корпуса файлов

    $ sydr-fuzz cmin -h
    Minimize corpus

    Usage: sydr-fuzz cmin [OPTIONS]

    Options:
      -r, --remove-corpus  Remove corpus-old directory after minimization
      -h, --help           Print help

Опция **-r, \--remove-corpus** удаляет директорию `corpus-old` с итоговым
не минимизированным корпусом. Если минимизации корпуса завершилась с ошибкой,
то директория не удаляется. В случае AFL++ и Crusher также удаляются файлы из
очередей всех инстансов (output_dir/aflplusplus/*-worker/queue/*).
...
### Опции генерации HTML отчета о покрытии (C/C++/Rust/Python/Go/Java/JavaScript/C#/Lua)
    ...
    Options:
      ...
      -t, --timeout <SECONDS>  Timeout (in seconds) for target execution [set 0 to disable]
                               [default: 30]
      -h, --help               Print help
...
При сборе покрытия Java-кода можно указать пути до исходного кода (список путей,
разделенных через ':'). В таком случае информация об исходном коде будет отражена
в html отчете. Пути до исходного кода указываются либо через переменную окружения
`CASR_SOURCE_DIRS`:

    $ export CASR_SOURCE_DIRS=/dir/with/sources/1:/dir/with/sources/2
    $ sydr-fuzz cov-html

либо в параметре `source` таблицы [cov] конфигурационного файла:
```toml
[cov]
source = "/dir/with/sources/1:/dir/with/sources/2"
```
...
### Продвинутые опции сбора покрытия (Python)
    ...
    Arguments:
      ...
      [ARGS]...  Coverage run extra options after --

    Options:
      -h, --help  Print help

Дополнительные аргументы **<ARGS>**, которые можно перечислить после `--`,
представляют собой соответствующие опции и аргументы для комманды run
...
Путь до исходного кода также можно задать через параметр `source` таблицы [cov]
...
### Режимы сбора покрытия (Go)

В `sydr-fuzz` доступно два режима для сбора покрытия Go-кода:

Режим **legacy**. Сбор покрытия после фаззинга с помощью
...
Режим **native**. Доступен для Go-тулчейнов версии выше `1.21`. Чтобы собрать
покрытие данным способом:
...
Компилируем проект с указанными опциями:

    # go build -cover -covermode=atomic -coverpkg=./... cmd/webp/main.go

Опция `-coverpkg=` отвечает за то, какие участки кода будут покрываться:
- `./...` - все файлы, векурсивно вложенные внутрь текущей директории.
- `main` - только package main.
- `all` - все, включая стандартную библиотеку и GOPATH.
- `std` - аналогичен `all`, но без GOPATH.
- `cmd` - расширяется на команды проекта и их внутренние библиотеки.
- `tool` - расширяется на инструменты, определенные в файле go.mod текущего модуля.

Запускаем приложение на разных файлах из корпуса, указав в переменной окружения
`GOCOVERDIR` путь, куда будут записаны файлы покрытия:

    # GOCOVERDIR=/image/raw ./main

Объединяем покрытие в один файл и генерируем отчёт о покрытии:

    # go tool covdata textfmt -i=/image/raw -o=coverprofile
    # go tool cover -html=coverprofile -o coverage.html

**NOTE** запуск команды `go tool cover` должен быть в директории (поддиректории) проекта,
содержащей файл `go.mod`.
...
и python-afl), `honggfuzz`, `atheris`, `jazzer`, `jazzer.js`, `luzer`, `difuzz`
или `crusher`.
В случае ensemble-фаззинга могут присутствовать сразу несколько из перечисленных
директорий.

Директории `libFuzzer`, `atheris`, `jazzer`, `jazzer.js` и `luzer`
содержат логи для каждого запущенного процесса фаззера (cmin.log, fuzz-0.log,
fuzz-1.log и т.д.).

    libfuzzer(atheris,jazzer,jazzer.js,luzer)/
    ├── cmin.log
    └── fuzz-1.log
...
(`sydr-worker` требуется для синхронизации между AFL++ и Sydr). Каждая
worker-директория содержит директории с рабочим корпусом (`queue`), найденными
аварийными завершениями (`crashes`), зависаниями (`hangs`) и прочей информацией.
В директории `aflplusplus/logs` хранятся логи AFL++.
...
Директория `difuzz` является рабочей директорией для LibAFL-DiFuzz и содержит лог
запуска фаззера (`fuzzer.log`), директории с отсортированными аварийными завершениями
по целевым точкам (crashes_TARGET_NAME_i:TARGET_LINE), а также директорию для синхронизации
с Sydr-ом `sydr-worker`. При установленной опции `xmin = true` директории с отсортированными
крешами не содержат файлы с одинаковыми путями достижения целевых точек.

    difuzz
    ├── fuzzer.log
    ├── crashes_TARGET_NAME_1:TARGET_LINE/
    ...
    ├── crashes_TARGET_NAME_n:TARGET_LINE/
    └── sydr-worker/

Директория `crusher` является общей выходной директорией для Crusher и содержит
директории `crusher/FUZZ-MASTER_0` и `FUZZ-SLAVE_N` для каждого запущенного
fuzz-инстанса, а также директорию `FUZZ-IMPORT_INPUTS` для синхронизации с Sydr. Каждая
выходная директория содержит директории с рабочим корпусом (`queue`), найденными
аварийными завершениями (`crashes`), зависаниями (`hangs`) и прочей информацией.
В директории `crusher/logs` хранятся логи Crusher.
...
"[SYDR] Files reloaded by Honggfuzz: {}" для Honggfuzz,
"[SYDR] Files reloaded by LibAFL-DiFuzz: {}" для LibAFL-DiFuzz. Для libFuzzer
...
При гибридном фаззинге с AFL++/Honggfuzz/Crusher дополнительно выполняется минимизация
...
корпус и сохраняет себе только полезные инпуты. В случае AFL++/Honggfuzz/Crusher все
инпуты от Sydr cначала накапливаются в директории `output/sydr/sydr-bucket`, раз в минуту
...
Для AFL++ и Crusher минимизация выполняется с помощью утилиты afl-showmap и глобальной
битовой карты, объединяемой между всеми запусками Sydr. Для Honggfuzz - с помощью
запуска фаззера в режиме минимизации (--minimize) только на текущем наборе данных в
`sydr-bucket`. Если Honggfuzz на каких-то целях работает слишком медленно, как альтернативу
можно использовать libFuzzer-merge: путь для libFuzzer-обертки нужно указать в параметре
"ilbfuzzer\_merge\_target" таблицы [honggfuzz] конфигурационного файла.

Минимизация инпутов Sydr для AFL++/Honggfuzz/Crusher может быть полностью выключена
...
Минимизация корпуса выполняется с помощью соответствующих утилит используемых
фаззеров: -merge/-set\_cover\_merge (libFuzzer), afl-cmin (AFL++), --minimize
(Honggfuzz), ftk (Crusher). Для AFL++ минимизация корпуса может быть запущена в несколько
параллельных потоков, при условии что утилита `afl-cmin` поддерживает такую функцию.
Число параллельных потоков указывается с помощью параметра `cmin` в конфигурационном
файле. По умолчанию используется половина всех доступных ядер на машине.

Для AFL++/python-afl/Sharpfuzz/Crusher итоговый корпус для минимизации собирается из
output\_dir/corpus и очередей всех инстансов (aflplusplus|crusher/\*/queue), включая
Sydr. При указании опции --remove-corpus удаляются все файлы итогового корпуса (corpus-old)
и очередей инстансов, остается только минимизированный корпус (corpus).
...
При сборе покрытия Python-кода c python-afl можно указать путь до собcтвенной реализации обёртки над таргетом 
в параметре `target` таблицы [cov] конфигурационного файла.
В таком случае вместо собственной sydr-fuzz обёртки библиотека сбора покрытия Coverage.py
получит пользовательский target. 
При запуске он получит абсолютный путь до корпуса (в `sys.argv[1]`), каждый файл которого 
должен быть использован при сборе покрытия.

А также, возможно задать [cov.env] таблицу переменных окружения, которые 
будут использоваться при сборе покрытия и иметь приоритет выше переменных из [pyafl.env].

Например, использование обёртки может конфигурироваться так:
```toml
[cov]
target = "/fuzz/cov_target.py"
[cov.env]
    CLASSIC_ENVVAR = "XXX"
```
...
# Собранный через go-fuzz.
[cov]
target = "/target_cov.zip"
mode = "legacy"

# Собранный с Go версии выше 1.21.
[cov]
target = "/instrumented_target @@"
...
**source** - путь до директории с исходным кодом проекта, содержащей файл
`go.mod` (необходимо только для сбора покрытия Go-кода версии выше 1.21).

**mode** - позволяет выбрать режим сбора покрытия для Go-приложений. Значение `auto`
выбирает режим автоматически по версии Go-тулчейна - если версия выше `1.21`, то выбирается
режим `native`, иначе `legacy`. Подробнее о режимах `legacy` и `native` можно прочитать
в секции `Режимы сбора покрытия (Go)` выше. Если не указать данный параметр в таблице `[cov]`,
его значение будет `auto` по умолчанию.
...
Для классов отказов в обслуживании требуется дополнительный ручной анализ, для того чтобы установить, является ли аварийное завершение критичным или нет.
