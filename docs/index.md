...
помощью [Atheris](https://github.com/google/atheris) и фаззинг Java с
помощью [Jazzer](https://github.com/CodeIntelligenceTesting/jazzer).
...
- Гибридный фаззинг с помощью Sydr и одного из фаззеров (libFuzzer, AFL++),
  фаззинг Python (Atheris) и Java (Jazzer):
  `sydr-fuzz run`
...
Доступные опции:
    ...
    SUBCOMMANDS:
        ...
        run           Run hybrid fuzzing with Sydr and libFuzzer/AFL++, Python fuzzing
                          with Atheris, or Java fuzzing with Jazzer
...
### Опции запуска фаззинга
    ...
    $ sydr-fuzz run -h
    sydr-fuzz-run
    Run hybrid fuzzing with Sydr and libFuzzer/AFL++, Python fuzzing with Atheris, or Java
    fuzzing with Jazzer
...
### Опции генерации HTML отчета о покрытии (C/C++/Rust/Python/Go/Java)
...
`[aflplusplus]` (или список таблиц `[[aflplusplus]]`), `[atheris]`, `[jazzer]` и `[cov]`. Конфигурационный файл не может
одновременно содержать таблицы `[libfuzzer]` и `[aflplusplus]`, а также
`[atheris]` и `[jazzer]` с какой-либо другой таблицей (на примере ниже приведены все таблицы
для краткости).
...
файлом sydr-fuzz в директорию `/fuzz`. Для работы инструментов CASR необходима
возможность работы GDB. Для этого требуется опция `--cap-add=SYS_PTRACE`. Для
работы casr-san в докере требуется разрешить системный вызов `personality`.
Подробную информацию о разрешении системных вызовов в докере можно найти
[тут](https://docs.docker.com/engine/security/seccomp). Скачиваем стандартный
профиль seccomp по этой
[ссылке](https://github.com/moby/moby/blob/master/profiles/seccomp/default.json),
затем добавляем `personality` в список разрешенных системных вызовов в профиле.
Передать обновленный профиль при запуске можно следующей опцией: `--security-opt
seccomp=/path/to/seccomp/profile.json`. Для проброса лицензионного ключа
добавляем следующее:
`-v /var/hasplm:/var/hasplm -v /etc/hasplm:/etc/hasplm`. Эти директории нужны для
[пробрасывания](https://docs.sentinel.thalesgroup.com/ldk/LDKdocs/SPNL/LDK_SLnP_Guide/Appendixes/Docker_containers.htm)
драйвера Sentinel в докер), также в докер пробрасывается время системы. Для
поддержки сетевого ключа добавьте опцию `--network host`. Итак итоговая команда
запуска контейнера имеет следующий вид:

    $ docker run --cap-add=SYS_PTRACE --security-opt \
        seccomp=/path/to/seccomp/profile.json --network host -v \
        /etc/localtime:/etc/localtime:ro -v /var/hasplm:/var/hasplm -v \
        /etc/hasplm:/etc/hasplm --rm -it -v $PWD:/fuzz sydr-fuzz-target /bin/bash
...
## Сбор покрытия (Python/Java)

Sydr-fuzz позволяет собирать покрытие на корпусе для целевого Python или Java-кода,
...
`all` - проверка всех предикатов безопасности (по умолчанию).
