...
`[aflplusplus]` (или список таблиц `[[aflplusplus]]`), `[atheris]`, `[jazzer]`,
`[jazzer_js]`, `[sharpfuzz]`, и `[cov]`.  Конфигурационный файл задает настройки
...
Таблицы `[atheris]`, `[jazzer]` и `[jazzer_js]` несовместимы с какой-либо другой таблицей,
таблица `[sharpfuzz]` может одновременно находиться в файле только с таблицей `[cov]`,
...
включает минимизацию входных файлов, сгенерированных инструментом Sydr (значим
для всех фаззеров, кроме AFL++).
