...
`[aflplusplus]` (или список таблиц `[[aflplusplus]]`), `[honggfuzz]`, `[difuzz]`, `[atheris]`,
`[pyafl]`, `[jazzer]`, `[jazzer_js]`, `[sharpfuzz]`, `[luzer]`, и `[cov]`. Конфигурационный файл задает
...
другой таблицей,
таблицы `[sharpfuzz]`, `[luzer]` и `[pyafl]` могут одновременно находиться в файле только с таблицей `[cov]`,
...
[crusher]
path = "/path/to/crusher/bin_x86-64/fuzz"
target = "/target_afl @@"
args = "-t 5000 --input-dir /corpus"
jobs = 6
...
# Для инструментированного C/C++/Rust кода.
[cov]
target = "/instrumented_target @@"

# Для инструментированного Go кода, собранного через go-fuzz.
[cov]
target = "/instrumented_target @@"
mode = "legacy"

# Для инструментированного Go кода версии выше 1.21.
[cov]
target = "/instrumented_target @@"
...
[difuzz]
target = "/target_libafl_difuzz"
args = "-j4 -i /corpus -e /ets.toml"
path = "/fuzzer_libafl_difuzz"
xmin = true
casr_bin = "/path/to/casr_bin"
[difuzz.env]
    LIBAFL_DEBUG_OUTPUT="1"
```

Таблица `[difuzz]` может содержать следующие параметры:

**path** - путь до бинарного файла фаззера LibAFL-DiFuzz. Данный параметр обязателен.

**args** - аргументы для LibAFL-DiFuzz. Необходимые аргументы - `-i` для указания входного корпуса
и `-e` для задания пути до файла `ets.toml`, содержащего информацию о целевых точках.
Также, можно задать следующие параметры:

* `-j` - число параллельных клиентов LibAFL-DiFuzz;
* `-p <число>`, чтобы указать определенный порт под конкретный запуск фаззера;
* `-l <число мегабайт>` для указания размера стека у фаззера (иногда нужно для больших проектов);
* `--forced`, если нужно, чтобы при гибридном фаззинге в корпус добавлялись все тестовые данные от
  Sydr (а не только те, которые LibAFL-DiFuzz посчитает интересными);
* `--exit-on-all`, для завершения фаззинга по достижении всех целевых точек;
* `--iters <число>` - число итераций фаззера;
* `--cool-time <число>` - время фазы набора покрытия;
* `--beta <число>` - параметр, обозначающий порог значимости метрики gMaxCov;
* `--sync-jobs` - число параллельных клиентов LibAFL-DiFuzz, выполняющих синхронизацию;
* `--sync-limit` - максимальное число файлов от Sydr, обрабатываемых за цикл синхронизации;
* `--panic-analysis [rust, go]` - если Rust или Go код завершается panic-ом, фаззер обрабатывает
  инпут как ошибку;
* ` --crash-exitcode <число>` - код возврата, который будет обрабатываться фаззером как ошибка.

Аргументы `-x` (выходная директория с найденными ошибками) и `-s` (директория синхронизации с Sydr)
задавать не нужно, они выставляются автоматически.

**target** - строка запуска фаззинг цели. Вместо имени входного файла следует использовать `@@`.
Если обертка читает входные данные со стандартного потока ввода, то `@@` указывать не нужно.

**xmin** - булевый параметр (по умолчанию `true`), который включает минимизацию
результатов фаззинга. Если параметр выключен, то минимизация результатов не производится,
а все файлы сохраняются в выходной директории.

**casr_bin** - путь до бинарного файла, собранного не через обертку `libafl_cc` или `libafl_rustc`
или без инструментации `goinstr_difuzz` (для Go-приложений).
Данный аргумент обязателен, если будет использоваться команда `sydr-fuzz casr` для анализа аварийных
завершений и `sydr-fuzz security` для проверки фаззинг цели на предикаты безопасности.

**[difuzz.env]** - таблица, в которой можно выставлять переменные окружения для LibAFL-DiFuzz.
Каждая строка таблицы задаёт строковую переменную, имя которой совпадает с именем
соответствующей переменной окружения. Переменная `LIBAFL_DEBUG_OUTPUT="1"` выставляется
автоматически. Также по умолчанию задаются переменные среды:

`ASAN_OPTIONS="hard_rss_limit_mb=2048,abort_on_error=1,detect_leaks=0,malloc_context_size=0,symbolize=0,allocator_may_return_null=1"`
`UBSAN_OPTIONS="halt_on_error=0,abort_on_error=0,malloc_context_size=0,allocator_may_return_null=1"`

Значения по умолчанию можно менять путем переопределения их в таблице `[difuzz.env]` или в системных
переменных окружения.
...
Таблица `[crusher]` может содержать следующие параметры:

**path** - путь до бинарного файла fuzz инструмента Crusher (crusher/bin\_x86-64/fuzz).
В случае если данный парамент в таблице не задан, то путь до инструмента ищется либо в PATH,
либо относительно запущенного бинарного файла sydr-fuzz (в случае, если Sydr-fuzz является частью
поставки Crusher и находится в crusher/bin\_x86-64/sydr). Путь до утилиты ftk, используемой
для минимизации корпуса, берется относительно найденного бинарного файла fuzz.

**target** - строка запуска обертки для Crusher (возможно использование обертки для AFL++).
Вместо имени входного файла следует использовать `@@` либо `__DATA__`. Если обертка читает входные
данные со стандартного потока ввода, то `@@` указывать не нужно.

**args** - аргументы Crusher. Необходимым аргументом является опция `-i/--input-dir` для
указания входного корпуса. Дополнительно можно указать опции для инструментации (`-I`/`--instrument`),
тайм-аутов (`-t 5000` миллисекунд), словарей (`--dictionary`), ограничения памяти (`-m`),
текстового баннера (`--text-banner`) и транспорта (`-T`/`--transport`, поддерживается только передача входных
данных через файл и стандартный поток ввода: file/stdin). Все остальные опции к указанию запрещены. Аргументы
с выходной директорией output-dir, master/slave, radamsa, no-ui, resume и import-first устанавливаются
автоматически - их указание в args также запрещено.

**cmin** - булевый/целочисленный параметр (по умолчанию `true`), который включает минимизацию
входного корпуса при старте гибридного фаззинга. Если параметр выключен (`false` или `0`),
то минимизация корпуса перед запуском гибридного фаззинга не производится. Минимизация корпуса
производится в один поток с помощью утилиты ftk.

**jobs** - число процессов Crusher для запуска (по умолчанию 1). При запуске нескольких процессов
автоматически выставляются опции --master/--slave для синхронизации.

**[crusher.env]** - таблица, содержащая значения переменных окружения, которые будут
выставлены для каждого запускаемого процесса Crusher. Каждая строка таблицы задаёт
строковую переменную, имя которой совпадает с именем соответствующей переменной окружения. Также
по умолчанию задаются переменные среды:

`ASAN_OPTIONS="detect_odr_violation=0,abort_on_error=1,detect_leaks=0,malloc_context_size=0,symbolize=0,allocator_may_return_null=1,handle_segv=0,handle_sigbus=0,handle_abort=0,handle_sigill=0,handle_sigfpe=0"`
`UBSAN_OPTIONS="malloc_context_size=0,symbolize=0,allocator_may_return_null=1,handle_segv=0,handle_sigbus=0,handle_abort=0,handle_sigill=0,handle_sigfpe=0"`

Значения по умолчанию можно менять путем переопределения их в таблице `[crusher.env]` или в системных
переменных окружения.
...
jobs = 6
venv = "/pyAflVenv/"
path = "/usr/local/bin/py-afl-fuzz"
cmin = true
ld_preload = true
target_function = "MyTestOneInputFunc"
[pyafl.env]
    ASAN_OPTIONS = "allocator_may_return_null=0"
```

Таблица `[pyafl]`, имеет такие же параметры **args**, **target**, **cmin** и **jobs**, что и `[aflplusplus]`, а также:

**path** - путь до py-afl-fuzz - shell обёртки от python-afl над afl-fuzz. Параметр не обязателен, если путь до директории, содержащей бинарник,
(стандартно `/usr/local/bin/`) добавлен в `$PATH` (в докере `sydr/ubuntu20.04-sydr-fuzz` уже добавлен) или же при использовании venv
(при активации venv `{venv_path}/bin/` добавляется в `$PATH`).
Предполагается, что py-afl-fuzz и py-afl-cmin лежат в одной директории.

**venv** - путь до желаемого для использования виртуального окружения, в котором 
будет запускаться таргет и прочие команды.

**ld_preload** - булевый параметр (по умолчанию включен), который позволяет при
запуске Python-кода для фаззинга и при минимизации входного корпуса
устанавливать в переменную среды `AFL_PRELOAD` путь до библиотеки
`libclang_rt.asan-[x86_64,x86,aarch64].so`, необходимой при работе с кодом, интрументированным при помощи asan.

**target_function** - строка (по умолчанию `TestOneInput`) содержащая название функции, которая
является входной точкой для фаззинга во время сбора покрытия (`sydr-fuzz pycov`, `sydr-fuzz cov-html`).

**[pyafl.env]** - таблица, содержащая значения переменных окружения, которые будут
выставлены для каждого запускаемого процесса python-afl. Каждая строка таблицы задаёт
строковую переменную, имя которой совпадает с именем соответствующей переменной окружения.
Например, чтобы использовать персистентный режим, можно указать PYTHON_AFL_PERSISTENT="1".

По умолчанию задаётся переменная среды `ASAN_OPTIONS="hard_rss_limit_mb=0,abort_on_error=1,detect_leaks=0,malloc_context_size=0,allocator_may_return_null=1"`

При сборе покрытия (`sydr-fuzz pycov`, `sydr-fuzz cov-html`), а также при анализе аварийных завершений (`sydr-fuzz casr`)  в `ASAN_OPTIONS` добавляется параметр `verify_asan_link_order=0`, который,
отключает проверку внутри `asan` на факт линковки его до прочих библиотек во время выполнения
(требуется из-за невозможности использовать `AFL_PRELOAD` в этих контекстах).

Также, возможно самостоятельно задать `LD_PRELOAD` значением пути до `libclang_rt.asan-[x86_64,x86,aarch64].so`
при запуске анализа аварийных завершений, для более точных результатов - но, это может нестабильно работать
на части систем.

Значения по умолчанию можно менять путем переопределения их в таблице `[pyafl.env]` или в системных
переменных окружения.
При этом наибольший приоритет имеют значения, заданные в файле конфигурации; средний приоритет имеют
значения, заданные в системных переменных окружения; низший приоритет имеют значения по умолчанию.
...
**use_ft_coverage** - как у libFuzzer.
...
**cmin**, **set_cover_merge** и **use_ft_coverage** - как у libFuzzer.
...
libFuzzer. Путь до корпуса указывается в аргументах Jazzer.js, т.е. до разделителя.
Опции Jazzer.js -m/mode, -d/dry_run, --coverage нельзя указывать - они выставляются
автоматически при сборе покрытия.
...
завершений устанавливать переменную среды `LD_PRELOAD` равной пути до библиотеки
`libclang_rt.asan-[x86_64,x86,aarch64].so`, необходимой при работе с кодом, имеющим расширения на
языках C/C++, для их инструментации.
...
### Coverage (для инструментированного C/C++/Rust/Go)
...
Для Go (собранного через go-fuzz):
...
`go.mod` (необходимо только для сбора покрытия Go-кода версии выше 1.21 в режиме `native`).
...
в секции `Режимы сбора покрытия (Go)` в документации по инструменту `sydr-fuzz`.
Если не указать данный параметр в таблице `[cov]`, его значение будет `auto` по умолчанию.
...
**source** - пути до исходного кода (список путей, разделенных через ':').

Аналогично пути до исходного кода можно указать через переменную окружения
`CASR_SOURCE_DIRS`:
    $ export CASR_SOURCE_DIRS=/dir/with/sources/1:/dir/with/sources/2

Таблица **[cov.env]** в случае Java-кода не используется.

### Coverage (для python-afl)
...
**target** - строка запуска обёртки для сбора покрытия фаззинг цели.
Её будет вызывать библиотека для сбора покрытия Coverage.py.
При запуске будет передан абсолютный путь до корпуса, каждый файл которого 
должен быть использован при сборе покрытия.
Параметр не обязателен, так как за исключением уникальных случаев
стандартная обёртка для сбора покрытия используемая в sydr-fuzz отрабатывает корректно.
...
Значения переменных окружения можно задавать также в системных переменных окружения.
При этом значения, заданные в файле конфигурации, более приоритетны, чем
значения, заданные в системных переменных окружения. А значения в **[cov.env]**
будут иметь приоритет выше переменных из **[pyafl.env]**.
